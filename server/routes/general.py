# ====================================================================================================
# server/routes/general.py — COMPLETE with custom system prompt support
# ====================================================================================================
"""
...
from middleware.auth import get_current_user, get_project_auth
...
# ================================================================================================
# CHAT — Public (API Key auth, accepts custom system prompt)
# ================================================================================================
...
async def chat_public(
    ...
    auth=Depends(get_project_auth("general")),
):
    """Public chat — uses General project API key. No user login required. Supports custom system prompts."""
...
async def chat(
    ...
    """Multi-turn AI chat with conversation history. Requires user login."""
    result = await chat_service.chat(
...
async def chat_stream(
    ...
    async def generate():
        ...
        try:
            conversation = None
            if conv_id: conversation = await Conversation.get(conv_id)
            if not conversation:
                conversation = Conversation(user_id=user.get("sub"), title=message[:50] + ("..." if len(message) > 50 else ""), interface=interface)
                await conversation.insert(); conv_id = str(conversation.id)

            await Message(conversation_id=conv_id, role=MessageRole.USER, content=message).insert()

            history = await Message.find(Message.conversation_id == conv_id).sort(-Message.timestamp).limit(20).to_list()
            history.reverse()
            ...
            if deep_think: system_prompt += " Use chain-of-thought reasoning."
            ...
            if provider == "gemini":
                ...
                async for chunk in ai_service.groq_chat_stream(messages_list, temperature=temperature, max_tokens=max_tokens_val, service="general"):
                    full_response += chunk
                    yield f"data: {json.dumps({'chunk': chunk, 'conversation_id': conv_id})}\n\n"
                yield f"data: {json.dumps({'done': True, 'conversation_id': conv_id})}\n\n"

            await Message(conversation_id=conv_id, role=MessageRole.ASSISTANT, content=full_response).insert()
            conversation.message_count += 2; conversation.updated_at = datetime.utcnow(); conversation.last_message = full_response[:100]
...
async def list_conversations(user: dict = Depends(get_current_user)):
    return SuccessResponse(data=await chat_service.get_conversations(user.get("sub")))

@router.get("/conversations/{conversation_id}")
async def get_conversation(conversation_id: str, user: dict = Depends(get_current_user)):
    return SuccessResponse(data=await chat_service.get_messages(conversation_id))

@router.delete("/conversations/{conversation_id}")
async def delete_conversation(conversation_id: str, user: dict = Depends(get_current_user)):
    success = await chat_service.delete_conversation(conversation_id, user.get("sub"))
    if not success: raise HTTPException(404, "Conversation not found")
...
async def learn(request: LearnRequest, user: dict = Depends(get_current_user)):
    result = await learn_service.learn(user_id=user.get("sub"), topic=request.topic, subject=request.subject, level=request.level, message=request.message, session_id=request.session_id)
    return SuccessResponse(data=result)

@router.get("/learn/{session_id}/quiz")
async def get_quiz(session_id: str, user: dict = Depends(get_current_user)):
    return SuccessResponse(data=await learn_service.get_quiz(session_id))
...
async def submit_quiz(session_id: str, request: QuizAnswerRequest, user: dict = Depends(get_current_user)):
    return SuccessResponse(data=await learn_service.submit_answer(session_id, request.question_index, request.answer_index))

@router.get("/learn/{session_id}/flashcards")
async def get_flashcards(session_id: str, user: dict = Depends(get_current_user)):
    return SuccessResponse(data=await learn_service.get_flashcards(session_id))
...
async def execute(request: CodeExecuteRequest, user: dict = Depends(get_current_user)):
    ...
    if not result.get("success"): raise HTTPException(400, result.get("error", "Execution failed"))
    return SuccessResponse(data=result)

@router.get("/execute/languages")
async def get_languages():
    return SuccessResponse(data=await execute_service.get_supported_languages())
...
async def generate_image(request: ImageGenerateRequest, user: dict = Depends(get_current_user)):
    ...
    if not result.get("success"): raise HTTPException(400, result.get("error", "Generation failed"))
    return SuccessResponse(data=result)
...
async def analyze_image(request: ImageAnalyzeRequest, user: dict = Depends(get_current_user)):
    ...
    if not result.get("success"): raise HTTPException(400, result.get("error", "Analysis failed"))
    return SuccessResponse(data=result)
...
async def generate_variations(request: ImageAnalyzeRequest, user: dict = Depends(get_current_user)):
    ...
    if not result.get("success"): raise HTTPException(400, result.get("error", "Failed"))
    return SuccessResponse(data=result)
