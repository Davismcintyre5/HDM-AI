# ====================================================================================================
# 34. server/services/general/analyze_service.py
# ====================================================================================================
...
class AnalyzeService:
    ...
    async def analyze(self, content: str, analysis_type: str = "summary") -> Dict[str, Any]:
        if analysis_type not in self.VALID_TYPES: return {"success": False, "error": f"Invalid type. Choose: {', '.join(self.VALID_TYPES)}"}
    ...
    async def _sentiment(self, content: str) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Analyze sentiment. Return JSON: {{\"sentiment\": \"positive/negative/neutral\", \"score\": -1 to 1, \"confidence\": 0.0-1.0, \"explanation\": \"...\"}}.\nText: {content}"}], temperature=0.1, max_tokens=300, service="general")
        try: return {"success": True, "result": json.loads(result.get("reply", "{}")), "analysis_type": "sentiment"}
        except: return {"success": True, "result": {"sentiment": "neutral", "score": 0}, "analysis_type": "sentiment"}

    async def _keywords(self, content: str) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Extract top 10 keywords as JSON: {{\"keywords\": []}}.\nText: {content}"}], temperature=0.3, max_tokens=300, service="general")
        try: return {"success": True, "result": json.loads(result.get("reply", "{}")).get("keywords", []), "analysis_type": "keywords"}
        except: return {"success": True, "result": [], "analysis_type": "keywords"}

    async def _entities(self, content: str) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Extract named entities as JSON: {{\"entities\": [{{\"name\": \"...\", \"type\": \"...\"}}]}}.\nText: {content}"}], temperature=0.2, max_tokens=500, service="general")
        try: return {"success": True, "result": json.loads(result.get("reply", "{}")).get("entities", []), "analysis_type": "entities"}
        except: return {"success": True, "result": [], "analysis_type": "entities"}

    async def _extract_data(self, content: str) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Extract structured data as JSON.\nText: {content}"}], temperature=0.2, max_tokens=1000, service="general")
        try: return {"success": True, "result": json.loads(result.get("reply", "{}")), "analysis_type": "data"}
        except: return {"success": True, "result": {}, "analysis_type": "data"}

    async def _full_analysis(self, content: str) -> Dict[str, Any]:
        summary = await self._summarize(content); sentiment = await self._sentiment(content); keywords = await self._keywords(content)
