# ====================================================================================================
# 20. server/services/spark/moderation_service.py
# ====================================================================================================
...
class ModerationService:
    async def check_spam(self, text: str, user_id: str = None, data: dict = None) -> Dict[str, Any]:
        ...
        result = await ai_service.groq_chat([{"role": "user", "content": f"Is this spam?{context}\nReturn JSON: {{\"is_spam\": true/false, \"confidence\": 0.0-1.0}}.\nText: {text}"}], temperature=0.1, max_tokens=80, service="spark")
        try:
            d = json.loads(result.get("reply", "{}"))
            await ModerationLog(user_id=user_id, content_type="text", flagged=d.get("is_spam", False), category="spam", confidence=d.get("confidence", 0)).insert()
            return d
