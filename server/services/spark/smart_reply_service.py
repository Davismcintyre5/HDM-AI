# ====================================================================================================
# 18. server/services/spark/smart_reply_service.py
# ====================================================================================================
...
class SmartReplyService:
    ...
    async def reply(self, message: str, count: int = 3, tone: str = None, data: dict = None) -> Dict[str, Any]:
        ...
        prompt += " One per line."
        result = await ai_service.groq_chat([{"role": "user", "content": prompt}], max_tokens=150, service="spark")
    ...
    async def quick_reply(self, message: str, count: int = 4, data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Generate {count} very short replies (1-3 words) to: '{message}'. One per line."}], max_tokens=80, service="spark")
    ...
    async def reply_with_tone(self, message: str, target_tone: str = "friendly", data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Reply to '{message}' in a {target_tone} tone."}], max_tokens=200, service="spark")
        return {"reply": result.get("reply", "")}

    async def reply_in_language(self, message: str, language: str = "en", data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Reply to '{message}' in {language}."}], max_tokens=200, service="spark")
        return {"reply": result.get("reply", "")}
