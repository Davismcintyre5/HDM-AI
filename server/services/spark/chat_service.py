# ====================================================================================================
# 8. server/services/spark/chat_service.py
# ====================================================================================================
...
class SparkChatService:
    ...
    def _build_context(self, message: str, data: Optional[dict] = None) -> str:
        """Build context prompt from real data."""
        if not data:
            return ""
    ...
    async def summarize(self, text: str, max_length: int = 200, data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Summarize in {max_length} chars: {text}"}], max_tokens=300)
    ...
    async def voice_chat(self, transcript: str, language: str = "en", data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "system", "content": f"Respond naturally in {language}."}, {"role": "user", "content": transcript}], max_tokens=500)
        return {"response_text": result.get("reply", "")}

    async def emoji_suggest(self, message: str, count: int = 3, data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Suggest {count} emojis for: {message}. Return only emojis."}], max_tokens=50)
