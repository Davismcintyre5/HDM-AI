# ====================================================================================================
# 17. server/services/spark/chat_service.py
# ====================================================================================================
...
class SparkChatService:
    ...
    async def ask(self, user_id: str, message: str, language: str = "en", data: dict = None) -> Dict[str, Any]:
        ...
        result = await ai_service.groq_chat([{"role": "system", "content": system + context}, {"role": "user", "content": message}], max_tokens=800, service="spark")
    ...
    async def translate(self, text: str, target_language: str, data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Translate to {target_language}: {text}"}], max_tokens=500, service="spark")
    ...
    async def voice_chat(self, transcript: str, language: str = "en", data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "system", "content": f"Respond naturally in {language}."}, {"role": "user", "content": transcript}], max_tokens=500, service="spark")
        return {"response_text": result.get("reply", "")}

    async def emoji_suggest(self, message: str, count: int = 3, data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Suggest {count} emojis for: {message}. Return only emojis."}], max_tokens=50, service="spark")
    ...
    async def format_message(self, text: str, format_type: str = "markdown", data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Format in {format_type}: {text}"}], max_tokens=500, service="spark")
        return {"formatted": result.get("reply", text)}

    async def quote_reply(self, original: str, reply: str, data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Original: {original}\nReply: {reply}\nCombine into quoted reply."}], max_tokens=300, service="spark")
        return {"reply": result.get("reply", "")}

    async def poll_generate(self, topic: str, options_count: int = 4, data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Create a poll about '{topic}' with {options_count} options. Return JSON: {{question, options:[]}}"}], max_tokens=200, service="spark")
