# ====================================================================================================
# 28. server/services/vibe/monetization_service.py
# ====================================================================================================
...
class VibeMonetizationService:
    async def ad_target(self, campaign_goal: str, budget: float = None, audience: dict = None, data: dict = None) -> Dict[str, Any]:
        ...
        result = await ai_service.groq_chat([{"role": "user", "content": prompt}], max_tokens=300, service="vibe")
    ...
    async def ad_copy(self, product: str, target_audience: str, platform: str = "instagram", data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Write ad copy for {product} targeting {target_audience} on {platform}"}], max_tokens=300, service="vibe")
    ...
    async def price_suggest(self, product: str, category: str, market_data: dict = None, data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Suggest pricing for {product} in {category}. Return JSON: {{\"price\": 0}}"}], max_tokens=150, service="vibe")
    ...
    async def sponsor_match(self, creator_profile: dict, niche: str, data: dict = None) -> Dict[str, Any]:
        result = await ai_service.groq_chat([{"role": "user", "content": f"Match sponsors for {niche} creator. Return JSON: {{\"matches\": []}}"}], max_tokens=300, service="vibe")
