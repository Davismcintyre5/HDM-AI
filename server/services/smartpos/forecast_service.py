# ====================================================================================================
# 9. server/services/smartpos/forecast_service.py
# ====================================================================================================
...
class ForecastService:
    async def forecast(self, business_id: str, forecast_type: str, period: str = "next_month", data: dict = None, product_ids: list = None) -> Dict[str, Any]:
        ...
        if "current_stock" in data:
            ...
            for item in data["current_stock"][:10]: parts.append(f"  • {item.get('product','?')}: Stock {item.get('stock',0)}, Daily Sales {item.get('daily_sales',0)}, Lead Time {item.get('lead_time_days',0)}d")
        ...
        parts.append("⚠️ Use specific product names and numbers from the data.")

        prompt = "\n".join(parts)
        result = await ai_service.groq_chat([{"role": "user", "content": prompt}], temperature=0.3, max_tokens=600, service="smartpos")
        try:
